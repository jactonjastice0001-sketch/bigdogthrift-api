...
// Default country (used when number is in local format)
const DEFAULT_COUNTRY = process.env.DEFAULT_COUNTRY || 'KE';

// 2. SINGLE-PARSE HELPER
...
const sendSMS = async (phoneNumber, message, countryCode = null) => {
    try {
        ...
        return {
            success: false,
            error: 'Unexpected response format from Africa\'s Talking',
            data: response
        };
...
const sendSMSWithRetry = async (phoneNumber, message, maxRetries = 3, countryCode = null) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        console.log(`Attempt ${attempt}/${maxRetries} to send SMS to ${phoneNumber}`);
